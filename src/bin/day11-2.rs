...
fn main() {
    ...
    let mut divisible_lcm = monkeys[0].test.divisible;
    for monkey in monkeys.iter() {
        divisible_lcm = lcm(divisible_lcm, monkey.test.divisible);
    }
    // println!("{}", divisible_lcm);

    let mut inspection_cnts: Vec<u64> = vec![0; monkeys.len()];
